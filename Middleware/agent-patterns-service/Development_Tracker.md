...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, selectable via `useCase`): **LangGraph 50/50 ✅ COMPLETE.** Now propagating to the other 7 frameworks, one pattern per cycle. Each pattern's use-case instructions live in the pattern's `_base.py` (`USE_CASES` catalog) so every framework cell pulls the SAME catalog and differs only in mechanics. **CrewAI:** 40/50 — Reflection ✅ + Evaluator-optimizer ✅ + Chaining ✅ + Router ✅ + Tree-of-Thoughts ✅ + ReWOO ✅ + Multi-agent ✅ + Plan-execute ✅ (all verified live; each pattern's 5 use cases share a `_base` catalog with LangGraph — plan-execute uses a planner agent for LLM plans + a deterministic execute (corpus/vehicle_spec) + a synthesizer agent; step counts match LangGraph exactly). **Left:** rag, react (tool/retrieval-heavy). `GET /agent-patterns/{pattern}/usecases` lists them.
