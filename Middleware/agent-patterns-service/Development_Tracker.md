...
**Microsoft Agent Framework:** 25/50 — Reflection ✅ + Tree-of-Thoughts ✅ + Multi-agent ✅ + Chaining ✅ + Router ✅ (all verified live via `msa.acomplete` over the shared `_base` catalogs; each cell runs ALL its calls in ONE event loop via `msa.run_sync(_go())` — AF telemetry ContextVar breaks across repeated `asyncio.run`). **Left (MS):** evaluator, rewoo, plan-execute, rag, react.
