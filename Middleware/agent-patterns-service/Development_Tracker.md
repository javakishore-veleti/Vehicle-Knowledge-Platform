...
**Google ADK:** 40/50 — Reflection ✅ + Tree-of-Thoughts ✅ + Multi-agent ✅ + Chaining ✅ + Router ✅ + Evaluator-optimizer ✅ + ReWOO ✅ + Plan-execute ✅ (all verified live via `adk.complete` over the shared `_base` catalogs; rewoo plan sizes 2/9/6/3/8 + plan-execute steps 4/5/3/6/5 match the other cells, query-rewriter scores via REAL retrieval). **Left (Google ADK):** rag, react (native LlmAgent + FunctionTool).
**Next frameworks for the use-case axis:** Microsoft, Strands (0/50 each). `GET /agent-patterns/{pattern}/usecases` lists them.
