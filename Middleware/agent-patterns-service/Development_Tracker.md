...
**OpenAI Agents:** ✅ **50/50 COMPLETE** — all 10 patterns × 5 use cases verified live over the shared `_base` catalogs via `oa.complete`, with the native Agent + Runner where it fits: RAG → Agent + use-case-scoped `@function_tool` (sources match the other frameworks), ReAct → Agent + per-use-case `@function_tool` subset.
...
**Google ADK:** ✅ **50/50 COMPLETE** — all 10 patterns × 5 use cases verified live over the shared `_base` catalogs via `adk.complete`, with the native LlmAgent + InMemoryRunner where it fits: RAG → LlmAgent + use-case-scoped `FunctionTool` (sources match the other frameworks), ReAct → LlmAgent + per-use-case `FunctionTool` subset.
**Microsoft Agent Framework:** ✅ **50/50 COMPLETE** — all 10 patterns × 5 use cases verified live over the shared `_base` catalogs via `msa.acomplete` (each cell in ONE event loop), with the native Agent + @tool where it fits: RAG → Agent + use-case-scoped `@tool` (sources match the other frameworks), ReAct → Agent + per-use-case `@tool` subset.
**AWS Strands:** ✅ **50/50 COMPLETE** — all 10 patterns × 5 use cases verified live via `sa.complete` (synchronous Strands Agent over OpenAIModel) over the shared `_base` catalogs, with the native Agent + @tool where it fits: RAG → Agent + use-case-scoped `@tool` (sources match the other frameworks), ReAct → Agent + per-use-case `@tool` subset.

### ✅✅ USE-CASE AXIS 400/400 COMPLETE — all 10 patterns × 5 VKP use cases × 8 frameworks
LangGraph · CrewAI · LlamaIndex · Haystack · OpenAI Agents · Google ADK · Microsoft Agent Framework · AWS Strands — every cell verified live. Each pattern's use-case behavior lives in ONE shared `app/patterns/<pattern>/_base.py` catalog (prompts + deterministic helpers + scoping/route/plan/tool data), consumed by all 8 framework cells, so the framework columns differ ONLY in mechanics. Deterministic outputs (rewoo blind-plan sizes 2/9/6/3/8, chaining sha256/majority/chunk-count, RAG scoped sources, plan-execute step counts) come out identical across all 8 frameworks; native constructs are used where each SDK has them (LlamaIndex VectorStoreIndex+MetadataFilters / ReActAgent / SubQuestionQueryEngine, Haystack BM25 Pipeline+filters / Agent+Tool, and Agent+function-tool for the four agent SDKs). `GET /agent-patterns/{pattern}/usecases` lists them.
