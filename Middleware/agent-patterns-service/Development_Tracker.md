...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, 50 total): selectable via `useCase` in the request. **Done: 35/50** — Reflection + RAG + Evaluator-optimizer + Router + Plan-Execute + Multi-agent + ReWOO, ×5 each (LangGraph), all verified live (ReWOO plans all tool calls blind; nightly-price-refresh is fully LLM-free). `GET /agent-patterns/{pattern}/usecases` lists them.
