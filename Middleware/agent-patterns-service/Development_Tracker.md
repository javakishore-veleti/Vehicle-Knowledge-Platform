...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, selectable via `useCase`): **LangGraph 50/50 ✅ COMPLETE.** Now propagating to the other 7 frameworks, one pattern per cycle. Each pattern's use-case instructions live in the pattern's `_base.py` (`USE_CASES` catalog) so every framework cell pulls the SAME catalog and differs only in mechanics. **CrewAI:** 15/50 — Reflection ✅ + Evaluator-optimizer ✅ + Chaining ✅ (all verified live; chaining shares `_base` deterministic helpers so sha256/majority/chunk-count match LangGraph exactly). `GET /agent-patterns/{pattern}/usecases` lists them.
