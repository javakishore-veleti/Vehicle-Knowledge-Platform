...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, selectable via `useCase`): **LangGraph 50/50 ✅ COMPLETE.** Now propagating to the other 7 frameworks, one pattern per cycle. Each pattern's use-case instructions live in the pattern's `_base.py` (`USE_CASES` catalog) so every framework cell pulls the SAME catalog and differs only in mechanics. **CrewAI:** 30/50 — Reflection ✅ + Evaluator-optimizer ✅ + Chaining ✅ + Router ✅ + Tree-of-Thoughts ✅ + ReWOO ✅ (all verified live; each pattern's 5 use cases share a `_base` catalog with LangGraph — rewoo's blind-plan sizes match exactly, nightly-price-refresh fully LLM-free). **Left:** plan-execute, multi-agent, rag, react. `GET /agent-patterns/{pattern}/usecases` lists them.
