...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, 50 total): selectable via `useCase` in the request. **Done: 20/50** — Reflection + RAG + Evaluator-optimizer + Router, ×5 each (LangGraph), all verified live (Router builds the classify→conditional-edge graph per use case; topic-router blocks unsafe). `GET /agent-patterns/{pattern}/usecases` lists them.
