...
**Google ADK:** 25/50 — Reflection ✅ + Tree-of-Thoughts ✅ + Multi-agent ✅ + Chaining ✅ + Router ✅ (all verified live via `adk.complete` — LlmAgent through InMemoryRunner, routed via LiteLLM→OpenAI — over the shared `_base` catalogs). **Left (Google ADK):** evaluator, rewoo, plan-execute, rag, react.
