...
**AWS Strands:** 40/50 — Reflection ✅ + Tree-of-Thoughts ✅ + Multi-agent ✅ + Chaining ✅ + Router ✅ + Evaluator-optimizer ✅ + ReWOO ✅ + Plan-execute ✅ (all verified live via `sa.complete` — synchronous Strands Agent over OpenAIModel — over the shared `_base` catalogs; rewoo plan sizes 2/9/6/3/8 + plan-execute steps 4/5/3/6/5 match the other cells, query-rewriter scores via REAL retrieval). **Left (Strands):** rag, react (native Agent + @tool). `GET /agent-patterns/{pattern}/usecases` lists them.
