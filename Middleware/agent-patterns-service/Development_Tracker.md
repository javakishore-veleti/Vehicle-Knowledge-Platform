...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, selectable via `useCase`): **LangGraph 50/50 ✅ COMPLETE.** Now propagating to the other 7 frameworks, one pattern per cycle. Each pattern's use-case instructions live in the pattern's `_base.py` (`USE_CASES` catalog) so every framework cell pulls the SAME catalog and differs only in mechanics. **CrewAI:** 20/50 — Reflection ✅ + Evaluator-optimizer ✅ + Chaining ✅ + Router ✅ (all verified live; router shares `_base` route tables — classifier agent routes, LLM routes run a specialist agent, static routes return the routing string; topic-guardrail blocks unsafe). `GET /agent-patterns/{pattern}/usecases` lists them.
