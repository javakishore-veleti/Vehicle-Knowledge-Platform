...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, selectable via `useCase`): **LangGraph 50/50 ✅ COMPLETE.** Now propagating to the other 7 frameworks, one pattern per cycle. Each pattern's use-case instructions live in the pattern's `_base.py` (`USE_CASES` catalog) so every framework cell pulls the SAME catalog and differs only in mechanics. **CrewAI:** 35/50 — Reflection ✅ + Evaluator-optimizer ✅ + Chaining ✅ + Router ✅ + Tree-of-Thoughts ✅ + ReWOO ✅ + Multi-agent ✅ (all verified live; each pattern's 5 use cases share a `_base` catalog with LangGraph — multi-agent fans each use case's roster into a native crew + lead, per-brand-workers spins one agent per brand in the query). **Left:** plan-execute, rag, react. `GET /agent-patterns/{pattern}/usecases` lists them.
