...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, selectable via `useCase`): **LangGraph 50/50 ✅ COMPLETE.** Now propagating to the other 7 frameworks, one pattern per cycle. Each pattern's use-case instructions live in the pattern's `_base.py` (`USE_CASES` catalog) so every framework cell pulls the SAME catalog and differs only in mechanics. **CrewAI:** 25/50 (halfway) — Reflection ✅ + Evaluator-optimizer ✅ + Chaining ✅ + Router ✅ + Tree-of-Thoughts ✅ (all verified live; each pattern's 5 use cases share a `_base` catalog with LangGraph). `GET /agent-patterns/{pattern}/usecases` lists them.
