...
**Microsoft Agent Framework:** 40/50 — Reflection ✅ + Tree-of-Thoughts ✅ + Multi-agent ✅ + Chaining ✅ + Router ✅ + Evaluator-optimizer ✅ + ReWOO ✅ + Plan-execute ✅ (all verified live via `msa.acomplete`, ONE event loop per cell; rewoo plan sizes 2/9/6/3/8 + plan-execute steps 4/5/3/6/5 match the other cells, query-rewriter scores via REAL retrieval). **Left (MS):** rag, react (native Agent + @tool).
**Next frameworks for the use-case axis:** AWS Strands (0/50). `GET /agent-patterns/{pattern}/usecases` lists them.
