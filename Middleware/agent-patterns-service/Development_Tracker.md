...
- **Use-case axis** (the 5 concrete VKP use cases per pattern, 50 total): selectable via `useCase` in the request. **Done: 40/50** — Reflection + RAG + Evaluator-optimizer + Router + Plan-Execute + Multi-agent + ReWOO + ToT, ×5 each (LangGraph), all verified live (ToT branches→scores→selects per use case; multi-constraint-filter picks a vehicle meeting all constraints). `GET /agent-patterns/{pattern}/usecases` lists them.
