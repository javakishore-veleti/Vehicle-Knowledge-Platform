"""Prompt chaining / parallelization on **LlamaIndex** — chains + fan-outs driven by its LLM.

Implements the 5 VKP use cases via ctx['useCase']: multi-provider/sectioning/voting fan out across
li.complete calls; ingestion/translate are deterministic chains with one LLM step. Prompts + the
...
def _multi_provider(q):
    outs = [(n, li.complete(f"{sysp}\n\nQuestion: {q}")) for n, sysp in _base.PROVIDERS]
    body = "\n\n".join(f"{n}: {t}" for n, t in outs)
    return {"answer": li.complete(_base.CONSENSUS_PROMPT.format(body=body)), "steps": [n for n, _ in _base.PROVIDERS]}


def _ingestion(q):
    clean = _base.clean_text(q)
    title = li.complete(_base.TITLE_PROMPT.format(c=clean[:500])).strip()
