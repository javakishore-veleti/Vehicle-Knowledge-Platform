"""Prompt chaining / parallelization on the **OpenAI Agents SDK** — chains + fan-outs of Agent runs.

Implements the 5 VKP use cases via ctx['useCase']: multi-provider/sectioning/voting fan out across
Agent runs; ingestion/translate are deterministic chains with one LLM step. Prompts + the deterministic
...
def _multi_provider(q):
    outs = [(n, oa.complete(q, sysp)) for n, sysp in _base.PROVIDERS]
    body = "\n\n".join(f"{n}: {t}" for n, t in outs)
    return {"answer": oa.complete(_base.CONSENSUS_PROMPT.format(body=body)), "steps": [n for n, _ in _base.PROVIDERS]}


def _ingestion(q):
    clean = _base.clean_text(q)
    title = oa.complete(_base.TITLE_PROMPT.format(c=clean[:500])).strip()
