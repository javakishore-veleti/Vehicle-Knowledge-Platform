"""Prompt chaining / parallelization on **Google ADK** — chains + fan-outs of LlmAgent runs.

Implements the 5 VKP use cases via ctx['useCase']: multi-provider/sectioning/voting fan out across
LlmAgent runs; ingestion/translate are deterministic chains with one LLM step. Prompts + the deterministic
steps (clean/hash/section-split/sentence-chunk/majority) come from `_base` (shared with every framework
cell), so sha256/chunk-count/majority match the other cells exactly."""
from ... import registry, adk
from . import _base


def _multi_provider(q):
    outs = [(n, adk.complete(q, sysp)) for n, sysp in _base.PROVIDERS]
    body = "\n\n".join(f"{n}: {t}" for n, t in outs)
    return {"answer": adk.complete(_base.CONSENSUS_PROMPT.format(body=body)), "steps": [n for n, _ in _base.PROVIDERS]}


def _ingestion(q):
    clean = _base.clean_text(q)
    title = adk.complete(_base.TITLE_PROMPT.format(c=clean[:500])).strip()
