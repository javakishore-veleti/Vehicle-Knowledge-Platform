...
def run(ctx: dict) -> dict:
    q = ctx["input"]
    raw = adk.complete(f"Propose 3 DISTINCT candidate answers to: {q}. Separate each with '---'.")
    thoughts = [p.strip() for p in raw.split("---") if p.strip()][:3] or [raw]
    scores = []
    for t in thoughts:
        r = adk.complete(f"Rate 1-10 how well this answers '{q}'. Reply only the number.\n\n{t}", "You are a strict judge.")
        mm = re.search(r"\d+", r)
