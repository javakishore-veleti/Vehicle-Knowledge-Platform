...
def run(ctx: dict) -> dict:
    ...
    writer = Agent(role="Automotive Writer", goal="Answer vehicle questions accurately and concisely.",
                   backstory="A precise automotive expert.", verbose=False)
    critic = Agent(role="Fact Critic", goal="Find inaccuracies and gaps in automotive answers.",
                   backstory="A meticulous reviewer of vehicle claims.", verbose=False)
    ...
    crew = Crew(agents=[writer, critic], tasks=[draft_t, critique_t, revise_t],
