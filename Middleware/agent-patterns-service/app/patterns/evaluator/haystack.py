"""Evaluator-optimizer on **Haystack** — generate → evaluate → refine once if the score gate is unmet.

Implements the 5 VKP use cases via ctx['useCase']. The generate/refine prompts + eval strategy come
from `_base.USE_CASES` (shared with every framework cell): Haystack's generator does generate/refine +
the LLM judge, while retrieval / single-pass eval reuse `_base` (query-rewriter scores via REAL retrieval)."""
...
def run(ctx: dict) -> dict:
    ...
    draft = hay.complete(f"{system}\n\n{prompt}" if system else prompt)

    if spec["eval"] in ("retrieval", "single"):
        score, feedback = _base.evaluate(spec, q, draft)
    else:
        score, feedback = _base._parse_score(hay.complete(spec["judge"](q, draft)))

    refined = score < 8 and spec.get("refinable")
    answer = hay.complete(spec["refine"](q, draft, feedback)) if refined else draft
